...
const SCHEDULE: Record<string, ClassItem[]> = {
  ...
  Jumat: [
    { name: "Praktikum Komputasi Awan", time: "08.00 – 10.30", room: "Lab Jaringan", color: "cyan" },
  ],
};
...
function useTasks(initial: Task[]) {
  const [tasks, setTasks] = useState<Task[]>(() => {
    try {
      ...
      return saved ? (JSON.parse(saved) as Task[]) : initial;
...
// ─── AddTaskModal ─────────────────────────────────────────────────────────────
function AddTaskModal({ onClose, onAdd, initial }: { onClose: () => void; onAdd: (t: Omit<Task, "id" | "done">) => void; initial?: Task }) {
...
// ─── Section: Semua Tugas ─────────────────────────────────────────────────────
function TugasSection({ tasks, setTasks, showModal, setShowModal, searchQuery }: {
  ...
  function addTask(t: Omit<Task, "id" | "done">) {
    if (editTask) {
      setTasks(prev => prev.map(task => task.id === editTask.id ? { ...task, ...t } : task));
      setEditTask(null);
    } else {
  ...
  const searched = searchQuery?.trim()
    ...
    : tasks;
  const filtered = filter === "all" ? searched : filter === "pending" ? searched.filter(t => !t.done) : searched.filter(t => t.done);
    ...
    return (
      <div className="rounded-xl p-3.5 flex items-center gap-3 transition-all"
        ...
        <div className="flex flex-col gap-1.5 flex-shrink-0">
          ...
          <button onClick={() => { setEditTask(t); setShowModal(true); }} className="w-6 h-6 rounded-lg flex items-center justify-center transition-colors"
            title="Edit tugas"
            style={{ color: "#4a5e82" }}
  ...
  return (
    <>
      {showModal && <AddTaskModal onClose={() => { setShowModal(false); setEditTask(null); }} onAdd={addTask} initial={editTask ?? undefined} />}
      <div className="flex flex-col gap-4 p-4 sm:p-5 overflow-y-auto flex-1">
        ...
        {/* filter tabs */}
        <div className="flex gap-2 overflow-x-auto pb-1" style={{ scrollbarWidth: "none" }}>
          {([{ key: "all", label: "Semua" }, { key: "pending", label: "Belum Selesai" }, { key: "done", label: "Selesai" }] as const).map(({ key, label }) => (
            <button key={key} onClick={() => setFilter(key)}
              className="px-3 py-1.5 rounded-lg text-xs font-semibold transition-all flex-shrink-0"
              style={{ background: filter === key ? "rgba(0,229,255,0.14)" : "rgba(255,255,255,0.04)", border: filter === key ? "1px solid rgba(0,229,255,0.35)" : "1px solid rgba(0,229,255,0.08)", color: filter === key ? "#00e5ff" : "#4a5e82" }}>
              {label}
            </button>
          ))}
        </div>
        {tasks.length === 0 && (
          <div className="flex flex-col items-center justify-center flex-1 gap-3 py-16">
            ...
            <button onClick={() => { setEditTask(null); setShowModal(true); }} className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold"
...
// ─── Section: Dashboard ───────────────────────────────────────────────────────
function DashboardSection({ tasks, setTasks, setSection, showModal, setShowModal, searchQuery }: {
  tasks: Task[]; setTasks: React.Dispatch<React.SetStateAction<Task[]>>;
  ...
  const uniqueSubjects = new Set(tasks.map(t => t.subject)).size;

  const filtered = searchQuery.trim()
    ? pendingTasks.filter(t => t.title.toLowerCase().includes(searchQuery.toLowerCase()) || t.subject.toLowerCase().includes(searchQuery.toLowerCase()))
    : pendingTasks;
    ...
    <>
      ...
      <div className="flex-1 p-4 sm:p-5 overflow-y-auto">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 mb-4">
          <MetricCard value={String(uniqueSubjects || 7)} label="Mata Kuliah" />
          <MetricCard value={String(uniqueSubjects * 3 || 22)} label="Kredit SKS" accent />
          <MetricCard value={totalCount > 0 ? `${pct}%` : "—"} label="Progres Tugas" />
          <MetricCard value={`${pendingTasks.length} tugas`} label="Belum Selesai" accent />
        </div>
...
function DashboardPage({ onLogout }: { onLogout: () => void }) {
  ...
  const [tasks, setTasks] = useTasks([
    { id: 1, title: "Makalah E-Government Kota Bandung", subject: "E-Government dan Kota Cerdas", deadline: "2026-07-15", done: false },
    { id: 2, title: "Presentasi Proposal Proyek Akhir", subject: "Pengelolaan Proyek Teknologi Informasi", deadline: "2026-07-18", done: false },
    { id: 3, title: "Kuis Mingguan Komputasi Awan", subject: "Komputasi Awan", deadline: "2026-07-10", done: true },
    { id: 4, title: "Resume Jurnal Kewirausahaan", subject: "Kewirausahaan Teknologi", deadline: "2026-07-20", done: false },
    { id: 5, title: "Laporan Akhir Konsep Arsitektur", subject: "Konsep Arsitektur dan Perusahaan Sistem 1", deadline: "2026-07-25", done: false },
    { id: 6, title: "Prototipe Sensor IoT", subject: "Aplikasi Internet of Things", deadline: "2026-07-22", done: false },
  ]);
  ...
  function renderContent() {
    switch (activeSection) {
      case "dashboard": return <DashboardSection tasks={tasks} setTasks={setTasks} setSection={setActiveSection} showModal={showModal} setShowModal={setShowModal} searchQuery={searchQuery} />;
      case "jadwal":    return <JadwalSection />;
      case "tugas":     return <TugasSection tasks={tasks} setTasks={setTasks} showModal={showModal} setShowModal={setShowModal} searchQuery={searchQuery} />;
        ...
        <header className="flex items-center justify-between px-4 sm:px-6 py-3 flex-shrink-0"
          ...
          <div className="flex items-center gap-2 sm:gap-4 flex-shrink-0 ml-2">
            <div className="hidden sm:flex items-center gap-2 px-3 py-2 rounded-xl" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(0,229,255,0.14)", minWidth: 180 }}>
              ...
              <input className="bg-transparent outline-none text-sm w-full" placeholder="Cari tugas, mata kuliah..." style={{ color: "#e8f0fe" }} value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
            </div>
...
function LoginPage({ onLogin }: { onLogin: () => void }) {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError("");
    if (!nim.trim() || !pass.trim()) { setError("NIM dan Password wajib diisi."); return; }
    setLoading(true);
    setTimeout(() => {
      if (nim.trim() === VALID_CREDENTIALS.nim && pass.trim() === VALID_CREDENTIALS.password) {
        setLoading(false); onLogin();
      } else {
        setLoading(false); setError("NIM atau Password salah.");
      }
    }, 900);
  }
